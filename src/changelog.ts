...
export const changelogEntries: { date: string; items: ChangelogItem[] }[] = [
  {
    ...
    items: [
      ...
      { text: '新增密码登录验证，以及分离用户模式和游客模式', children: ['登录模式：可新增、编辑、删除、分享点位', '游客模式：可查看和分享该 ID 的点位数据，新增/编辑/删除入口隐藏'] },
      '修复关闭编辑视图卡界面问题',
      '修复蚂蚁线BUG',
  ...
  },
  {
    ...
    items: [
      ...
      { text: '分享页面定制', children: ['右侧栏：全屏地图', '左侧栏：地图、阵营、点位标题、点位图片'] },
      '添加点位分享功能',
      { text: '更新UI布局，使用三栏布局', children: ['右侧栏：查看点位、新增点位', '中间栏：全屏地图', '左侧栏：地图、阵营、英雄、技能'] },
      '优化冗余的代码以及没有被引用的数据库',
      '拆分App.tsx，优化结构',
