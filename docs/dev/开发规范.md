...
项目采用 MPA（多页应用）架构，包含三个独立应用：

```
src/apps/
├── shared/     # 共享库（首页） → index.html
├── user/       # 个人库 → user.html
└── admin/      # 管理后台 → admin.html
```

每个应用有独立的入口文件和页面组件，共享通用组件和 Hooks。

### 2. 入口文件规范

每个应用的入口文件（如 `shared/App.tsx`）只允许包含：
- ✅ 全局 `Provider`
- ✅ 路由/视图切换
- ✅ 应用初始化
